class EventsController < ApplicationController
  ...
  before_action :admin_only, only: %i[show destroy]
  ...
  def index
    ...
    @events = if current_user.try(:admin?)
                Event.all.unscoped.order('created_at DESC')
  ...
  def create
    ...
    @event.user_id = current_user.id unless current_user.try(:admin?)
  ...
  # admin only
  def admin_only
    redirect_to passenger_path(id: @event.passenger_id) unless user_signed_in? && current_user.admin?
  end
